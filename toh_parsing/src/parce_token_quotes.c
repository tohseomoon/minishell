...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   parce_token_quotes.c                               :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: toh <toh@student.42seoul.kr>               +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/06/30 13:44:11 by toh               #+#    #+#             */
/*   Updated: 2021/06/30 13:44:12 by toh              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		s_quote_str(t_token_util *utils, char quote, char **quote_str)
{
...
static int		d_quote_str(t_token_util *utils, char quote, char **quote_str)
{
	while (*(utils->str) != quote)
	{
		if (*(utils->str) == '\\' && *((utils->str) + 1) == quote
		&& *((utils->str) + 2) == 0)
			return (handle_quotes_error(ESCAPE));
		else if (*(utils->str) == '\\' && (*((utils->str) + 1) == '\\' ||
		*((utils->str) + 1) == '\"' || *((utils->str) + 1) == '$'))
		{
			(utils->str) += 1;
			(utils->cnt) += 1;
		}
...
int				quote_str(t_token_util *utils, char quote)
{
	...
	{
		if (s_quote_str(utils, quote, &quote_str) == -1)
			return (-1);
	}
	else if (quote == D_QUOTE)
	{
		if (d_quote_str(utils, quote, &quote_str) == -1)
			return (-1);
	}
